/**
 * Render project cards with smooth hover effects
 */
  ...
  container.innerHTML = projects
    .map(
      ...
      <article class="project-card" aria-labelledby="proj-${project.id}">
        <div class="project-icon" aria-hidden="true" style="color: ${project.color || '#4F6EF7'}">${ICONS[project.icon] || ''}</div>
        <h3 id="proj-${project.id}" class="project-title">${project.title}</h3>
        ...
        <footer class="project-footer">
          ...
          ${
            project.demo
              ? `<a href="${project.demo}" target="_blank" rel="noopener noreferrer" class="project-link" aria-label="View live demo of ${project.title}">
